package com.xuliang.tc.core.connect.proxy;
...
public class LcnConnectionProxy implements Connection {
    ...
    public RpcResponseState notify(int state) {
        try {
            if (state == 1) {
                log.info("commit transaction type[lcn] proxy connection:{}.", this);
                connection.commit();
            } else {
                log.info("rollback transaction type[lcn] proxy connection:{}.", this);
                connection.rollback();
            }
            connection.close();
            log.info("transaction type[lcn] proxy connection:{} closed.", this);
